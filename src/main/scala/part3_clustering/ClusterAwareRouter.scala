...
class MasterWithRouter extends Actor with ActorLogging {
  ...
  private val router = context.actorOf(FromConfig.props(Props[SimpleRoutee]), "clusterAwareRouter")
  override def receive: Receive = { case StartWork =>
...
class SimpleRoutee extends Actor with ActorLogging {
  override def receive: Receive = { case SimpleTask(contents) =>
...
object RouteesApp extends App {
  def startRouteeNode(port: Int) = {
    val config = ConfigFactory
      ...
      .withFallback(ConfigFactory.load("part3_clustering/clusterAwareRouter.conf"))
...
object MasterWithRouterApp extends App {
  val mainConfig = ConfigFactory.load("part3_clustering/clusterAwareRouter.conf")
